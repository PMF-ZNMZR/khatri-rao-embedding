...
%
% Testing routine for the LOPCG method, with folding to compute eigenvalues in the middle of the spectrum.

function [X, lam,res_vec,rank_vec,abs_error] = test_lobpcg_blk_folded(namedparams)
    arguments
...
function [X, lam,res_vec,rank_vec, example,abs_error] = run( config )
    ...
    temp_ref=load(strcat(strcat('./Lobpcg_blk_plot/ref_sol/matlab_','mat'),'.mat'));
    ref_sol=temp_ref.lam(1:1:3);
    clear temp_ref   

    [X, lam,res_vec,rank_vec,abs_error] = lrlobpcg_blk_folded( example, config, config.shift, ref_sol );
    t_end = toc(t_start);
