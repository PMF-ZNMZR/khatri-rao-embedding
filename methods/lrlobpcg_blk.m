function [X, lam,res_vec,rank_vec,abs_error] = lrlobpcg_blk( example, config, shift, ref_sol )
...
% [1] Kressner, Tobler - Preconditioned Low-Rank Methods for High-Dimensional Elliptic PDE Eigenvalue Problems (2011)
    ...
    maximal_rank=50;          % max rank of low-rank recompression of the columns during the process.

    config.tol_adi = 1e-15;    % Stop the ADI preconditioner when this rel. tolerance is reached.
    config.max_adi_iters = 8; % Maximum number of ADI iterations.
    ...
    maxiter =60;              % Maximum number of LOBPCG iterations.

    % Block size 
    blk_size=6;               % Block size of vectors.
    target_no=4;              % Tagret number of eigenpairs.
    ...
    for iter = 1:maxiter
        ...
        res_vec=[res_vec;res];
        abs_error=[abs_error;abs(ref_sol-lam(1:target_no)+example.shift)'];
        
        % stop if it reach the number of eigenpairs required such that residual < tol.
        ...
        % cast R1,R2 to Block low-rank format
...
function [AX1, AX2,AX_core] = applyA( example, X1, X2,X_core )
    % Compute A*X where A = kron(M, K) + kron(K, M) + kron(VR, VL) + shift*I.
...
function [U] = low_rank_truncate_without_qr( Z1, tol ,maximal_rank)
    ...
    nrmF = sqrt( sum(sigma.^2) );
    for r = 1:length(sigma)-1
        if( sqrt(sum(sigma(r+1:end).^2)) < tol * nrmF./sqrt(2) || r==maximal_rank )
            break;
        end
    end
...
function X_vec = block_Tensor_to_vec( X1, X2,X_core,blk_size )
    % return X_vec=[vec(X1*X2(:,:,1)*X3'), ..., vec(X1*X2(:,:,blk_size)*X3')] 
...
function [Ptilde1, Ptilde2] = preconditioner_ADI_one_by_one( example, config, P1, P2 ,blk_size)
    ...
    nsqrt = length( example.K );
    A1.X = example.M;
    A2.X = example.K + example.shift/2*speye(nsqrt);
